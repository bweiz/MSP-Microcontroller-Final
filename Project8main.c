...
/**
 * Ben Weizenegger, EELE 371, Project 8
 *  4/22/2024
 *
 */
...
int main(void)
{
    ...
    //--------------------Timer Setup----------------------
    TB0CTL |= TBCLR;        //clear timers and dividers; .75 sec == 26785(1/4)(1/7)
    TB0CTL |= TBSSEL__SMCLK;//source = SMCLK
    TB0CTL |= MC__UP;       //mode = up
    TB0CTL |= ID__4;        //devide by 4
    TB0EX0 |= TBIDEX__7;    //devide by 7
...
{
    if (sw == 1){
        if(position == sizeof(closedMessage)-1) {
            ...
            UCA1TXBUF = '/';
            for (i = 0; i < 500; i++){}
            UCA1TXBUF = ((Received[1] & 0xF0)>>4) + '0';    // Day 10s digit
            for (i = 0; i < 500; i++){}
            UCA1TXBUF = (Received[1] & 0x0F) + '0';         // Day 1s digit
            for (i = 0; i < 500; i++){}
            UCA1TXBUF = '/';
            for (i = 0; i < 500; i++){}
            UCA1TXBUF = ('2');
...
__interrupt void ADC_ISR(void){ //1.5 = (2^8x116)/3.3 max = 186.18
    ...
    if(ADC_Value > 186){
        ADC_Value = 186;                     //impelmenting max voltage limit
    }
    if(ADC_Value > 116){                     //if (A2 < 1.5v)
        voltageEnable = 1;                   //open switch enable code
    }
    if (ADC_Value < 116) {                 //if (A2 > 1.5 v)
...
//------------------Open Gate Start--------------------
void openGateSlow(){
    //still need to set up and conigure clock with stepper motor esp cnage fast clock speed.
    i = 0;                                    //reset counter
    TB0CCR0 = 271;                             //CCR0 = 271
